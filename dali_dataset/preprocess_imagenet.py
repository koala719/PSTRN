...
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, seed,
                 decoder_type, cache_size, cache_threshold, cache_type, dali_cpu=False, gpu_num=1):
        ...
        if decoder_type == "cached":
            ...
            self.input = ops.FileReader(file_root=data_dir, shard_id=device_id, num_shards=gpu_num, random_shuffle=True,
                                        stick_to_shard=True)
...
class HybridValPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, size, seed,
                 decoder_type, cache_size, cache_threshold, cache_type, dali_cpu=False,
                 gpu_num=1):
        ...
        if decoder_type == "cached":
            ...
            self.input = ops.FileReader(file_root=data_dir, shard_id=device_id, num_shards=gpu_num,
...
def get_imagenet_iter_dali(type, image_dir, batch_size, num_threads, gpu_num, crop, seed,
    ...
    if type == 'train':
        ...
        for device_id in range(gpu_num):
            ...
            iter_size_tmp = pip_train.epoch_size("Reader")
            iter_size =  iter_size_tmp // gpu_num
            if iter_size_tmp != iter_size * gpu_num:
                iter_size += 1
            dali_iter_tmp = Len_DALIClassificationIterator(pip_train,
                                                             size=iter_size,
                                                             fill_last_batch=False,
                                                             auto_reset=auto_reset,
                                                             )
            dali_iter_trains.append(dali_iter_tmp)
        return dali_iter_trains
    elif type == 'val':
        dali_iter_vals = []
        for device_id in range(gpu_num):
            ...
            iter_size_tmp = pip_val.epoch_size("Reader")
            iter_size = iter_size_tmp // gpu_num
            if iter_size_tmp != iter_size * gpu_num:
                iter_size += 1
            dali_iter_tmp = Len_DALIClassificationIterator(pip_val,
                                                           size=iter_size,
                                                           fill_last_batch=False,
                                                           auto_reset=auto_reset,
                                                           )
            dali_iter_vals.append(dali_iter_tmp)
        return dali_iter_vals
