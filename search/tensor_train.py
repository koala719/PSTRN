...
from model.lenet_tensor_fc import LeNet5_Fashion_Mnist_Tensor
...
def main(rank_code, no_cuda=False, seed=233, lr=2e-3, start_epoch=0, weight_conv_dir=None,
         ...
         epochs=20, save_freq=0, continual_save_freq=3, model="LeNet5_Mnist_Tensor_NANNAN_OURS",rank_ratio=None, code_bias=None, save=None):
    # ---------------------- Set GPU ----------------------
    ...
    # ------------------- Define Model ------------------------
    if model == "LeNet5_Fashion_Mnist_Tensor_OURS":
        rank_num = 7
    else:
        rank_num = 20
    ...
    for i in range(rank_num):
        rank = rank_code[i] * rank_ratio[i] + code_bias[i]
        if rank < 2:
            rank = 2
        RANK.append(rank)
    print('RANK:', RANK)
    ...
    print ("---", rank_code, "---")
    if model in Models:
        model = Models[model](num_classes=10, RANK=RANK)
    else:
