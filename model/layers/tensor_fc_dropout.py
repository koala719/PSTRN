...
class TensorRingLinear(nn.Module):
    def __init__(self, input_size: int, output_size: int, input_shape: list,
                 output_shape: list, rank_shape: list, bias: bool = True, is_dropout: bool = True, drop_rate=0.1):
        ...
        for node_shape in node_shapes:
            tmp = nn.Parameter(torch.Tensor(
                node_shape["shape"][0], node_shape["shape"][1]))
    ...
    def check_shape_setting(self):
        assert self.input_size == np.prod(
            self.input_shape), "The decomposition of the input_size is not suitable!"
    ...
    def generate_node_shapes(self):
        ...
        for i in range(self.nodes_num):
            ...
            elif i < self.nodes_num - 1 or self.dropout is not None:
                tmp = dict(
                    name="node%d" % i,
                    shape=(right_rank * middle_rank, left_rank)
                )
        ...
        for i in range(self.nodes_num):
            ...
            if i == 0:
                ...
                if self.dropout is None:
                    ...
                    # res: [b, O1O2O3, r0 r0], w: [r0 r0, 1] | res : [b, O1O2O3]
                    dropout_matrix = torch.diag(
                        self.dropout(torch.ones(right_rank, device='cuda')))
    ...
    def calculate_compression(self):
        ...
        param_tr = np.sum(
            self.tr_ranks_line[:-1] * self.tr_ranks_line[1:] * self.whole_node_shape)
...
class BTTuncker_FC(nn.Module):
        ...
        for i in range(self.block_num):
            ...
            for j in range(self.factor_num):
                factor_tmp = dict(
                    ...
                    shape=(
                        self.input_shape[j], self.output_shape[j], self.rank_shape[j])
                )
    ...
    def dec_multy(self, inputs, weights):
        ...
        for weight in weights:
            ...
            cal_tmp = cal_tmp.reshape(-1,
                                      self.core_size).matmul(core.view(self.core_size, -1))
